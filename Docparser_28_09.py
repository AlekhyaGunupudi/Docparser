...
def main_page():
    ...
    db_set = {'Database1','Database2'}
    name = st.text_input("Enter index name for your new database (optional):")       
    db_set.add(name)
    selection = st.selectbox('Select an existing database where you want to store your files',options=list(db_set))
...
def page2():
    ...
    name = st.text_input("Enter index name for your new database (optional):")
    selection = st.selectbox('Select an existing database where you want to store your files',('Database1','Database2',f'{name}'))
    ...
    if selection == f'{name}':
        if st.button("Click to get answer"):
            #st.header("Answer")
            if pdf_file is not None:
                files = Driver().upload_a_file_into_new_db(name,pdf_file)
                get_answer = Driver().answers(question,files)
                st.write(get_answer)
                st.write("saved in your selected database!")
